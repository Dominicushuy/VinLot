  ...
  const handleProcessAll = async () => {
    ...
    try {
      ...
      toast({
        title: 'Đang đối soát',
        description: isDemo
          ? 'Đang xử lý tất cả các cược đang chờ...'
          : 'Đang xử lý đối soát cho ngày đã chọn...',
      })
      ...
      if (result) {
        if (result.processed > 0) {
          toast({
            title: 'Thành công',
            description: `Đã đối soát ${result.processed} cược, ${
              result.won
            } cược thắng${
              result.skippedBets > 0
                ? `, bỏ qua ${result.skippedBets} cược chưa có kết quả`
                : ''
            }`,
            variant: 'lottery',
          })
          ...
          toast({
            title: 'Không có cược nào được đối soát',
            description:
              result.message ||
              'Không có cược nào cần đối soát hoặc chưa có kết quả xổ số tương ứng',
          })
        }

        // Nếu xử lý xong hết, thông báo
        if (result.processed + (result.skippedBets || 0) >= totalBets) {
          if (onAllProcessed) {
            onAllProcessed()
          }
        }
